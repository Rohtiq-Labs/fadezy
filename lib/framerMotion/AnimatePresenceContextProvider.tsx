...
const AnimatePresenceContextProvider: FC<Props> = ({children, pageTransitionType = "FullScreenSlide", mode}) => {
    ...
    useEffect(() => {
        if (pageTransitionType === "none" && events.animationEnd) {
            const timeout = setTimeout(() => document.dispatchEvent(events.animationEnd!), 50);
            return () => clearTimeout(timeout);
        }
    }, [key, pageTransitionType, events.animationEnd]);
    ...
    const PageTransitionComponent = FullScreenSlideTransition;
    return (
