...
const ServicesImpact: FC = () => {
  const lang = useCurrentLanguages();
  const { heading, subheading, items } = servicesContents[lang].impact;
        ...
        <FadeInScrollAnimation trigger={sectionRef} duration={1}>
          ...
          >
            {items.map((item, index) => (
              <li key={`services-impact-${index}`} className="flex flex-col gap-3">
                <span className="font-blinker font-light text-[clamp(36px,calc(((100vw-1024px)/896*14)+36px),56px)] leading-none text-brand-text tracking-[0.02em]">
                  {item.stat}
                </span>
