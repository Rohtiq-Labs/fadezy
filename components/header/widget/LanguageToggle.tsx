...
const LanguageToggle: FC = () => {
    ...
    return (
        <div className="cursor-pointer relative uppercase text-brand-text" ref={containerRef}>
            <div className="flex items-center gap-[4px]" onClick={handleMenuToggle}>
                <HeadingXsThick>{currentLang}</HeadingXsThick>
                <ChevronUp />
            </div>
            <ul
                className="absolute top-[calc(100%+16px)] left-1/2 -translate-x-1/2 shadow-card rounded-saas border border-brand-border overflow-hidden"
                style={{
                ...
                {supportedLanguages.map((lang, i) => (
                    <li key={`${lang}-${i}`}>
                        <span
                            ...
                            className={cn(
                                "inline-block px-[14.25px] py-[10px] bg-brand-bg text-brand-text w-full text-left",
                                { "bg-primary-light text-primary font-semibold": currentLang === lang }
                            )}
                                    style={{
